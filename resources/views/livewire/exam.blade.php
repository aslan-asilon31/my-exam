<div>
    ...
    @elseif ($examStarted && !$examFinished && !$examList)
        <!-- Exam Questions -->
        <div class="exam-container w-full bg-white shadow-md rounded-lg p-6 max-w-2xl mx-auto mt-10">

            <img src="{{ $questions[$currentQuestionIndex]['img_url'] }}" class="w-auto" alt="" srcset="">
            ...
            <p class="mt-4 text-red-700">{{ session('error') }}</p>
            ...
            <div class="timer mb-4">
                <p class="text-lg font-medium">Waktu Ujian Tersisa: 
                    {{-- <span id="exam-timer" class="font-bold text-blue-600">{{ gmdate('i:s', $examTimer) }}</span> --}}
                </p>
                <p class="text-lg font-medium">Waktu untuk soal ini: 
                    <span id="question-timer" class="font-bold text-blue-600">{{ $questions[$currentQuestionIndex]['timer'] }}</span> seconds
                </p>
            </div>

            <button onclick="startQuestionTimer({{ $questions[$currentQuestionIndex]['timer'] }})" wire:click="previousQuestion" class="bg-blue-500 m-1 p-1 radius-md text-white">Previous</button>
            <button onclick="startQuestionTimer({{ $questions[$currentQuestionIndex]['timer'] }})" wire:click="nextQuestion" class="bg-blue-500 m-1 p-1 radius-md text-white" >Next</button>

        
            <div class="navigation-buttons flex justify-between">
                @if ($currentQuestionIndex > 0)
                    <button wire:click="previousQuestion" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Sebelumnya</button>
                @endif
                @if ($currentQuestionIndex < count($questions) - 1)
                    <button wire:click="nextQuestion" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600" {{ empty($answers[$currentQuestionIndex]) ? 'disabled' : '' }}>Selanjutnya</button>
                @else
                    <button wire:click="finishExam" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600" {{ empty($answers[$currentQuestionIndex]) ? 'disabled' : '' }}>Selesaikan Ujian</button>
                @endif
            </div>
        </div>
